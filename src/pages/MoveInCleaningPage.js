...
function EditForm({ initial, onCancel, onSaved }) {
  ...
  // 드롭다운 옵션 (serviceSettings/입금자, serviceSettings/거래처)
  ...
  /* DatePicker를 인풋과 동일 크기 */
  const dateInputClass =
    "w-full h-10 px-3 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-400";
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") e.preventDefault(); // 자동 제출 방지
            }}
            className="w-full h-10 px-3 rounded-md border border-gray-300 text-right focus:outline-none focus:ring-2 focus:ring-purple-400"
          />
        </div>
...
export default function MoveInCleaningPage() {
      ...
      (snap) => {
        ...
        const list = snap.docs.map((d) => {
          ...
          return {
            ...
            vendor: s(x.vendor),
            status: s(x.status), // 표시 시 빈값이어도 StatusCell에서 '미접수'
            note: s(x.note),
            __depositNum: deposit,
            __payoutNum: payout,
            __settleYm: ym,                 // "YYYY-MM"
            __settleNum: dateToNum(settle), // 숫자형 정렬 보조값
            __sortCombo: sortCombo,         // 🔑 DataTable 정렬 키
          };
        });
  ...
  /* 🔁 B. 이사정산 → 입주청소 자동 동기화
        - 항상 모든 moveouts 문서를 반영
        - moveDate → settleDate
        - status가 '정산완료'일 때만 depositIn = cleaningFee, 그 외는 0
        - 진행현황이 비어있다면 '미접수'로 기본 설정 */
  useEffect(() => {
  ...
  // 진행현황 필터 반영 (정렬은 DataTable이 __sortCombo로 수행)
  ...
  const columns = useMemo(
    () => [
      ...
      // 정렬용 숨김 키는 데이터에만 존재하면 됨 (__sortCombo)
    ],
    []
  );
      ...
      >
        <EditForm
          ...
          onCancel={() => {
            setFormOpen(false);
            setEditingRow(null);
            setFormMode("create"); // ✅ 버그 수정: 함수 호출
          }}
          onSaved={handleSaved}
        />
      </SimpleModal>
